...
public class PolynomialFunctionTests {
    ...
    public void denseRandomMVQTest() {
        ...
        Assert.assertEquals(
                1 + f.getInputLength() + ( ( f.getInputLength() * ( f.getInputLength() - 1 ) ) >>> 1 ),
    ...
    public void bucketTest() {
        ...
        watch.reset();
        watch.start();
        BitVector[][] buckets = outer.bucket();
        ...
        for ( int i = 0; i < 25; ++i ) {
            ...
            Assert.assertEquals( innerResult, innerResultByMatrix );

            BitVector composedResult = new BitVector( outer.getOutputLength() );
            for ( int j = 0; j < buckets.length - 1; ++j ) {
                EnhancedBitMatrix AT = EnhancedBitMatrix.directFromRows( Arrays.asList( buckets[ j ] ) );
                if ( innerResult.get( j ) ) {
                    EnhancedBitMatrix inputMatrix = EnhancedBitMatrix.directFromRows( Arrays
                            .asList( new BitVector[] { BitVectors.affineExtend( randomInput ) } ) );
                    EnhancedBitMatrix coeffMatrix = ( j == 0 ? BT : BT.resizeColumns( BT.cols() - j ) ).multiply( AT )
                            .transpose();
                    // EnhancedBitMatrix resultMatrix = inputMatrix.multiply( coeffMatrix );
                    BitVector result = coeffMatrix.multiply( BitVectors.affineExtend( randomInput ) );
                    // BitVector result = AT.transpose().multiply(
                    // BT.transpose().multiply( BitVectors.affineExtend( randomInput ) ).partFromTo( j , BT.cols() - 1 )
                    // );
                    BitVector bucketResult = outer.bucketToFunction( j ).apply( innerResult );
                    Assert.assertEquals( bucketResult, result );
                    composedResult.xor( result );
                }
            }
            composedResult.xor( buckets[ buckets.length - 1 ][ 0 ] );
            logger.debug( "Random input: {}", randomInput );
            logger.debug( "Inner result: {}", innerResult );
            logger.debug( "Outer result: {}", outerResult );
            logger.debug( "Composed result: {}", composedResult );
            Assert.assertEquals( outerResult, composedResult );
        }
    }
    ...
    public void testGeneralComposeParameterized() {
        ...
        SimplePolynomialFunction inner = ParameterizedPolynomialFunctions.fromUnshiftedVariables(
                other.getInputLength(),
                other,
                pipelines );
    ...
    public void testConcatenateOutputs() {
        ...
        SimplePolynomialFunction lhs = SimplePolynomialFunctions.denseRandomMultivariateQuadratic(
                inputLength,
                outputLength );
        SimplePolynomialFunction rhs = SimplePolynomialFunctions.denseRandomMultivariateQuadratic(
                inputLength,
                outputLength );
    ...
    public void testInterleaveFunctions() {
        ...
        Pair<int[], int[]> inputMaps = SimplePolynomialFunctions.getSplitMap(
                lhs.getInputLength(),
                rhs.getInputLength() );
        Pair<int[], int[]> outputMaps = SimplePolynomialFunctions.getSplitMap(
                lhs.getOutputLength(),
                rhs.getOutputLength() );

        SimplePolynomialFunction interleaved = SimplePolynomialFunctions.interleaveFunctions(
                lhs,
                rhs,
                inputMaps.getLeft(),
                inputMaps.getRight(),
    ...
    public void testRandomlyPartitionMVQ() {
        ...
        Pair<SimplePolynomialFunction, SimplePolynomialFunction> gh = SimplePolynomialFunctions
    ...
    public void testPipelineStage() {
        ...
        SimplePolynomialFunction f = SimplePolynomialFunctions.denseRandomMultivariateQuadratic(
                inputLength,
                outputLength );
    ...
    public void testUnitPipeline() {
        ...
        logger.info(
                "Non-linear unit pipeline generation took {} ms",
                PIPELINE_LENGTH,
    ...
    public void mostFrequentFactorTest() {
        ...
        Map<Monomial, Set<Monomial>> memoizedComputations = OptimizedPolynomialFunctionGF2.initializeMemoMap(
                256,
                monomials,
                contributions );
        ...
        Monomial mostFrequent = OptimizedPolynomialFunctionGF2.mostFrequentFactor(
                monomials,
                possibleProducts.keySet(),
    ...
    public void testNonlinearPipeline() {
        ...
        Pair<SimplePolynomialFunction, SimplePolynomialFunction[]> pipelineDescription = SimplePolynomialFunctions
                .buildNonlinearPipeline( inner, functions );
        logger.info(
                "Non-linear pipeline generation of length {} took {} ms",
                PIPELINE_LENGTH,
                watch.elapsed( TimeUnit.MILLISECONDS ) );
